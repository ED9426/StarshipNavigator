...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      // traverse and add each element in copaPath to current dataSquence
      for (int i = 0; i < copyPath.dataSequence.size(); i++) {
        this.dataSequence.add(copyPath.dataSequence.get(i));
      }
      // add data of the vertex extend by the input edge
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    if (start == null || end == null) {
      throw new NoSuchElementException("No start or end vertex found");
    }
    ...
    // unchecked vertices
    List<Vertex> unchecked = new ArrayList<Vertex>();
    // the shortest path to be returned
    Path shortestPath = null;
    // set all the vertices as unchecked
    for (Vertex v: vertices.values()) {
      unchecked.add(v);
    }
    // set the start vertex as the current node
    Vertex currNode = vertices.get(start);
    // used to save and poll the shortest path
    PriorityQueue<Path> queue = new PriorityQueue<Path>();
    // used to save all the paths found
    PriorityQueue<Path> queueAll = new PriorityQueue<Path>();
    // add the current node to the queue
    queue.add(new Path(currNode));
    queueAll.add(new Path(currNode));

    // stops when each node is checked
    while (unchecked.size() != 0) {
      Path currPath = null; // current path
      Path toTarget = null; // the path from start to target
      currPath = queue.peek(); // set the shortest path as current path
      if (currPath == null) { // if 
        break;
      }
      // set node at end of current path as current node
      currNode = currPath.end;
      // find out all the edges of current node
      List<Edge> edges = currNode.edgesLeaving;

      // traverse all the edges
      for (int i=0; i<edges.size(); i++) {
        // set edge's target as target node
        Vertex targetNode = edges.get(i).target;
        // if unchecked
        if (unchecked.contains(targetNode)) {
          // find out if this node is visited before
          for (Path p: queueAll) {
            if (p.start == vertices.get(start)
                && p.end == targetNode) {
              toTarget = p;
              break;
            }
          }
          if (toTarget == null // if node not visited before
              || currPath.distance + getWeight(currNode.data, targetNode.data)
              < toTarget.distance) { // if finds shorter path
            // add the shorter path to the queue
            queue.add(new Path(currPath, edges.get(i)));
            queueAll.add(new Path(currPath, edges.get(i)));
          }
        }
      }
      // remove the checked node and shortest path
      unchecked.remove(currNode);
      queue.poll();
    }
    // find out the shortest path that starts and ends with parameters
    int queueSize = queueAll.size();
    for (int i=0; i<queueSize; i++) {
      Path temp = queueAll.poll();
      if (temp.start.data.equals(start) && temp.end.data.equals(end)) {
        shortestPath = temp;
        break;
      }
    }
    // throws exception if no path from start to end
    if (shortestPath == null) {
      throw new NoSuchElementException("There's no path from start to end.");
    } else {
      return shortestPath;
    }
  }
